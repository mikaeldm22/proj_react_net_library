...
export default function AcervoPage() {
  ...
  function getBooksWithSameTitle(title: string) {
    return books.filter(book => book.title.trim().toLocaleLowerCase() === title.trim().toLocaleLowerCase());
  }
  ...
  function getBooksByGenre(): Record<string, Book[]> {
  ...
  return (
    <div className="space-y-8">
      ...
      {books.length === 0 ? (
        ...
        <div className="space-y-8">
          {Object.entries(getBooksByGenre()).map(([genre, genreBooks]) => (
            <div key={genre}>
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <caption className="text-left text-lg font-bold text-gray-900 px-6 py-4 bg-gradient-to-r from-gray-50 to-gray-100">
                      📕 Gênero: {genre}</caption>
                      ...
                      {genreBooks.map((book) => (
                        <tr key={book.title} className='hover:bg-indigo-50 transition-colors duration-150'>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold ${getAvailableBooksCount(book.title) > 0
                              ? 'bg-green-100 text-green-800'
                              : 'bg-orange-100 text-orange-800'
                              }`}>
